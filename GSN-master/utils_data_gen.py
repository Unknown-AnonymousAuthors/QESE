...
def generate_dataset(data_path, 
    ...
    if 'ogb' in data_path:
        ...
        graphs = get_dataset(dataset_name, recal=False, cal_entro=True, nc_norm=1, ec_norm=10, eig="appro_deg_ge0")
        num_classes = graphs.num_classes
        # for graph in graphs:
        #     graph.edge_mat = graph.edge_index
        num_node_type, num_edge_type = None, None
...
def _prepare(data, subgraph_dicts, subgraph_params, regression, dataset_name, ex_fn, cnt_fn):
    ...
    setattr(new_data, 'graph_size', data.x.shape[0])
