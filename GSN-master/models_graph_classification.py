...
from graph_filters.GSN_sparse import GSN_sparse, QGSN_sparse
...
class GNNSubstructures(torch.nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(d_out)):
            ...
            if use_ids:
                ...
                if self.quantum:
                    filter_fn = QGSN_sparse
            else:
                filter_fn = MPNN_edge_sparse if use_efs else MPNN_sparse
            self.conv.append(filter_fn(**kwargs_filter)) # filter_fn = GSN_sparse # !!!
    ...
    def forward(self, data, print_flag=False, return_intermediate=False):
        ...
        for i in range(0, len(self.conv)):
            ...
            kwargs['node_centrality'] = data.node_centrality
            kwargs['edge_centrality'] = data.edge_centrality
