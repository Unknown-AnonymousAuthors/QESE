...
def main(args):
    ...
    for fold_idx in fold_idxs:
        ...
        if args['mode'] == 'train':
            ...
            train_losses, train_accs, test_losses, test_accs, val_losses, val_accs, best_acc = metrics
    ...
    best_acc = np.array(best_acc_l) * 100
    print('############# Training Over #############')
    result = f"Dataset: {args['dataset_name']}, Quantum: {args['quantum']}, Best Acc: {best_acc.mean().item():.2f}, Std: {best_acc.std().item():.2f}\n"
...
if __name__ == '__main__':   
    ...
    parser.add_argument('--dataset_name', type=str, default='MUTAG') # ['MUTAG', 'PTC_MR', 'PROTEINS', 'NCI1', 'NCI109', 'ENZYMES', 'IMDB-BINARY', 'IMDB-MULTI']
    parser.add_argument('--quantum', type=str, default='True')
    parser.add_argument('--nc_norm', type=float, default=1.0)
