...
class GNN_ogb(nn.Module):  # for molhiv
    def __init__(self, params):
        ...
        for layer in range(self.nlayers):
            if self.quantum:
                self.convs.append(QGNNLayer(self.nhid, self.nhid, params))
            else:
    ...
    def forward(self, h, edge_index, pair_info, batch, node_centrality=None, edge_centrality=None):

        h_list = [self.atom_encoder(h)]
        
        for layer in range(self.nlayers):
            if self.quantum:
                h = self.convs[layer](h_list[layer], pair_info, node_centrality, edge_centrality)
            else:
...
class GNN_bench(nn.Module):  # for zinc, csl and tu datasets
    ...
    def forward(self, h, edge_index, pair_info, batch, node_centrality=None, edge_centrality=None):
        ...
        for layer in range(len(self.convs)):
            if self.quantum:
                h = self.convs[layer](h, pair_info, node_centrality, edge_centrality)
            else:
