...
def main():
    ...
    parser.add_argument('--nc_norm', type=float, default=1., help='')
    parser.add_argument('--ec_norm', type=float, default=10., help='')
    parser.add_argument('--record_num', type=int, choices=[i+1 for i in range(10)])

    parser.add_argument('--d', type=int, default=3, # 1
    ...
    if args.quantum == 'True':
        dataset = get_dataset('ogbg-molhiv', recal=False, cal_entro=True, nc_norm=args.nc_norm, ec_norm=args.ec_norm, eig= "appro_deg_ge0")
        dataset.num_tasks, dataset.eval_metric = 1, 'rocauc'
        train_mask, test_mask, val_mask = torch.load(f"../datasets/obj/ogbg-molhiv_masks.pt")
    ...
    save_file = f'preprocessed/{args.dataset}_{args.d}_{args.t}_{args.connected}_{args.quantum}.data'
    try:
        print('Loading pair infomation...')
        time_t = time.time()
        train_loader, valid_loader, test_loader = torch.load(save_file, map_location=torch.device('cpu'))
    ...
    train_loader.pin_memory_device = []
    valid_loader.pin_memory_device = []
    test_loader.pin_memory_device = []
    ...
    params = {
        ...
        'keys': torch.load(f'preprocessed/{args.dataset}_{args.d}_{args.t}_{args.connected}_{args.quantum}_keys.data', map_location=torch.device('cpu')), # subgraph.get_keys_from_loaders([train_loader, valid_loader, test_loader])
    }

    record_path = f'./record/model_opt_epoch_{args.record_num}.pt'
    if os.path.exists(record_path):
        model, optimizer, epoch_already = torch.load(record_path, map_location=torch.device('cpu'))
    else:
        model = models.GNN_ogb(params).to(device)
        optimizer = optim.Adam(model.parameters(), lr=0.001)
        epoch_already = -1
        torch.save((model, optimizer, epoch_already), record_path)
    ...
    with tqdm(range(args.epochs)) as tq:
        for epoch in tq:
            ...
            tq.set_description(f'epoch: {epoch} time: {time.time()-time_t:.4f} train: {train_perf[dataset.eval_metric]:.4f} vali: {valid_perf[dataset.eval_metric]:.4f} test: {test_perf[dataset.eval_metric]:.4f} | best_val: {valid_curve[best_val_epoch]:.4f} best_test: {test_curve[best_test_epoch]:.4f}')
            
            torch.save((model, optimizer, epoch), record_path)

    best_val_epoch = np.argmax(np.array(valid_curve))
    best_test_epoch = np.argmax(np.array(test_curve))

    print('Best validation score: {}'.format(valid_curve[best_val_epoch]))
    print('Test score: {}'.format(test_curve[best_test_epoch]))
