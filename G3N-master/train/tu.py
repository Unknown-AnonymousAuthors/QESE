...
def train(train_loader, model, optimizer, device):
    ...
    for iter, data in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        output = model(x, edge_index, (pairs, degrees, scatter), batch_idx, node_centrality, edge_centrality)
    ...
    avg_loss /= (iter + 1)
    acc = (y_preds == y_trues).float().mean().item()
...
def test(loader, model, device):
    ...
    with torch.no_grad():
        for iter, data in enumerate(loader):
            ...
            output = model(x, edge_index, (pairs, degrees, scatter), batch_idx, data.node_centrality, data.edge_centrality)
            loss = F.cross_entropy(output, labels)
            avg_loss += loss.detach().item()
            y_preds.append(torch.argmax(output, dim=-1))
            y_trues.append(labels)
