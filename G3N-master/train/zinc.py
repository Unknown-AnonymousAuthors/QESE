...
def train(model, optimizer, loader, epoch, device):
    ...
    for iter, batch in enumerate(loader):
        ...
        node_centrality = batch.node_centrality.to(device)
        edge_centrality = batch.edge_centrality.to(device)

        optimizer.zero_grad()

        # zinc pos enc not used
        batch_scores = model(x, edge_attr, batch.edge_index, edge_feat_mat, (pairs, degrees, scatter), batch_idx, node_centrality, edge_centrality).squeeze(1)
