...
def main():
    ...
    # parser.add_argument('--dataset', type=str, required=False, choices=['EXP', 'CSL', 'graph8c', 'SR25'])
    parser.add_argument('--eig', type=str, required=False, default='np', choices=['np', 'appro_deg_ge0', 'betweenness', 'current_flow_betweenness'])
    parser.add_argument('--nc_norm', type=float, required=False, default=1.)
    parser.add_argument('--ec_norm', type=float, required=False, default=10.)
    ...
    for epoch in tq:
        ...
        tq.set_description('Epoch: {:02d}, trloss: {:.6f},  Valloss: {:.6f}, Testloss: {:.6f}, best test loss: {:.6f}, bestr2:{:.6f}, time:{:.6f}'.format(epoch,trloss,val_loss,test_loss,btest,btestr2,time_taken))
        
        if bval<1e-4:
            break
    print(f'Best test loss: {btest:.2e}')
