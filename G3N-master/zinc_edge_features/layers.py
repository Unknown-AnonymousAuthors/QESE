...
class QGNNLayer(nn.Module):
    ...
    def forward(self, h, edge_attr, edge_index, edge_feat_mat, pair_info, node_centrality, edge_centrality):
        ...
        for key in pairs:
            ...
            if self.combination == "multi":  # s(h_x @ W) * s(h_y @ W)
                ...
                for i in range(self.t):
                    ...
                    h_temp = h_temp * self.transform[k](h_t)
                    try:
                        h_temp *= edge_centrality.view(-1, 1) # !!!
                    except:
                        ...
            elif self.combination == "sum":  # s(h_x @ W + h_y @ W)
                h_temp = 0
            ...
            h_sum *= node_centrality.view(-1, 1) # !!!
