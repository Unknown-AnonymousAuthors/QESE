...
parser.add_argument('--nc_scale', type=float, default=1., help='')
parser.add_argument('--ec_scale', type=float, default=10., help='')
parser.add_argument('--record_num', type=int, choices=[i+1 for i in range(100)])

parser.add_argument('--d', type=int, default=3, # 4
...
dataset = get_dataset('ZINC_subset', recal=False, cal_entro=True, nc_scale=args.nc_scale, ec_scale=args.ec_scale, eig= "appro_deg_ge0")
# print(dataset[0], dataset[0].x)
trainset = dataset[:10000]
testset = dataset[10000:11000]
valset = dataset[11000:12000]

# print(trainset[0], trainset[0].x)
# assert False
...
record_path = f'./record/model_opt_sche_epoch_{args.record_num}.pt'
if os.path.exists(record_path):
    model, optimizer, scheduler, epoch_already = torch.load(record_path)
else:
    ...
    torch.save((model, optimizer, scheduler, epoch_already), record_path)
    ...
    with tqdm(range(args.epochs)) as tq:
        for epoch in tq:
            ...
            tq.set_postfix(lr=optimizer.param_groups[0]['lr'],
                          train_loss=epoch_train_loss, val_loss=epoch_val_loss, test_loss=epoch_test_loss, best_test=np.array(epoch_test_losses).min())

            per_epoch_time.append(time.time() - startime_t)

            scheduler.step(epoch_val_loss)
            
            torch.save((model, optimizer, scheduler, epoch), record_path)
            with open(f'./record/model_opt_sche_epoch_{args.record_num}.txt', 'a') as f:
                f.write(f'{epoch}, train:{epoch_train_loss}, test:{epoch_test_loss}, val:{epoch_val_loss}\n')
...
print("Train MAE: {:.4f}".format(np.array(epoch_train_losses)))
print("test MAE: {:.4f}".format(np.array(epoch_test_losses)))
print("Val MAE: {:.4f}".format(np.array(epoch_val_losses)))
