...
class GNN_bench(nn.Module):  # for zinc, csl and tu datasets
    ...
    def forward(self, h, edge_attr, edge_index, edge_feat_mat, pair_info, batch, node_centrality=None, edge_centrality=None):
        ...
        for layer in range(len(self.convs)):
            if self.quantum:
                h = self.convs[layer](h, edge_attr, edge_index, edge_feat_mat, pair_info, node_centrality, edge_centrality)
            else:
