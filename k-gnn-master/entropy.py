...
def cal_node_centrality(data_list: List[Data], order: int, eig: str, hop: int = 1) -> list:
    '''
        for sep
    ...
    for graph in tqdm(data_list, desc="Calculating node centrality", ncols=90, leave=False):
        if order == 1:
            ...
            edge_index = graph.edge_index_2
        elif order == 3:
            edge_index = graph.edge_index_3
        else:
            raise ValueError('order must be 1~3')
        
        adj_origin = _edgeindex2adj(edge_index, edge_index.max()+1)
...
def cal_edge_centrality(data_list, order: int, eig: str) -> list:
    '''
        for sep
